...
function() {
	...
	conf : {
		"ajaxBase" : "/graphs/agraph/tp/gremlin?script=",
		"ajaxTemplates" : {
	...
	responseProcessors : {
		...
		"promotions" : function(response) {
			...
			jQuery.each(response.results.slice(-16), function(index, value) {
	...
	shareMapView : function() {
		...
		me._shareMapViewImpl(maplinkArgs);

	},

	_shareMapViewImpl : function(maplinkArgs) {
		var me = this;
		var sandbox = this.getSandbox();
	...
	eventHandlers : {
		...
		"mapshare.ShareMap" : function(event) {
			var me = this, maplinkArgs = event.getMaplinkArgs();
